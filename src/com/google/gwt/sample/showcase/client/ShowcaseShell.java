...
public class ShowcaseShell extends ResizeComposite {
	...
	public void setContent(final ContentWidget content) {
		 ...
		 * innerHtml will close the select tag in the open tag (ie, use a
		 * forward slash instead of a separate close tag) which most browsers
		 * parse incorrectly.
		 */
		tabSourceList.clear();
	...
	private void showSourceFile() {
		...
		if (!tabSourceList.isVisible() || tabSourceList.getSelectedIndex() == 0) {
			// If the source list isn't visible or the first item is selected,
			// load
