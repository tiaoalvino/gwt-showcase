...
public class CwBidiFormatting extends ContentWidget {
	 ...
	 */
	@ShowcaseData
	private BidiFormatter bidiFormatter = BidiFormatter.getInstanceForCurrentLocale();
	...
	public CwBidiFormatting(CwConstants constants) {
		super(constants.cwBidiFormattingName(), constants.cwBidiFormattingDescription(), false, "BlogMessages.java",
	...
	public Widget onInitialize() {
		...
		// Add argument 0
		arg0Box = new TextBox();
		// Using an initial value whose direction is opposite the locale's
		// direction
		// demonstrates the need for and effect of bidi formatting.
		arg0Box.setText(LocaleInfo.getCurrentLocale().isRTL() ? "Tom Bombadil" : "תומר גרין");
		...
		// demonstrates the need for and effect of bidi formatting.
		arg2Box.setText(
				LocaleInfo.getCurrentLocale().isRTL() ? "How deep is your love?" : "כמה חול יש בחוף?");
	...
	private void updateMessages() {
		...
		bidiFormattedMessage.setHTML(blogMessages.userComment(bidiFormatter.spanWrap(arg0),
				// arg1 is intended to be an unsigned number, so bidi formatting
				// is not
				// needed. However, HTML escaping is a must to avoid an XSS
				// attack hole!
